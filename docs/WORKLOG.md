...
- **Last updated**: 2026-08-17 (Root-caused the dead demo prompt: an open `@radix-ui/react-dialog` sets `pointer-events: none` on `<body>`, and the prompt is portalled there, so every click fell through. Measured on /nearby — dialog open flips body pointer-events to `none`. It now waits for `[role="dialog"][data-state="open"]` to clear before opening, which is also the correct sequencing, and carries `pointer-events-auto` as a backstop. My previous stacking-context explanation was wrong; docs/11 corrected.)
