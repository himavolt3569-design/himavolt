...
- **Last updated**: 2026-08-17 (Demo Videos tab no longer uses OS dialogs — `window.confirm` on both deletes is replaced by the shared portalled `DeleteConfirmDialog`, and `window.prompt` for naming a section by an inline field with Enter/Escape. Verified live: no native dialog fires, copy names what is being removed, Cancel closes without touching anything.)
- **Previously**: 2026-08-17 (Demo Videos tab: five action handlers never cleared `error`, so a message about one section stayed pinned while you worked on another — deleting an empty "Random" looked like it was failing with a "Getting Started" error. Every action now clears first, and a section that still holds videos shows its delete control dimmed with `aria-disabled` and a label naming the count, instead of looking enabled and erroring on click.)
- **Previously**: 2026-08-17 (Two admin fixes: native `<select>` popups now follow the theme — a global `select option` rule in globals.css, since the open option list is browser-painted and ignores the control's classes, which is why a themed select dropped a white menu with a system-blue highlight onto the warm palette; and sections in the Demo Videos tab gained a delete control, guarded client-side and server-side against removing one that still holds videos. Both verified live in the admin panel.)
- **Previously**: 2026-08-17 (Master admin is no longer rate-limited out of its own login — credentials are compared before the limiter and a correct master pair skips it. Platform staff and wrong credentials still get 5/15min, verified live (401s then 429 with Retry-After). **This removes the lockout as a brute-force control on the master password**; a 300ms constant delay on every failure stands in for it, and `MASTER_ADMIN_PASSWORD` is now the whole boundary on the root account — see docs/03.)
- **Previously**: 2026-08-17 (Root-caused the dead demo prompt: an open `@radix-ui/react-dialog` sets `pointer-events: none` on `<body>`, and the prompt is portalled there, so every click fell through. Measured on /nearby — dialog open flips body pointer-events to `none`. It now waits for `[role="dialog"][data-state="open"]` to clear before opening, which is also the correct sequencing, and carries `pointer-events-auto` as a backstop. My previous stacking-context explanation was wrong; docs/11 corrected.)
- **Previously**: 2026-08-17 (Demo prompt now waits for setup — it fires only once the operator owns a restaurant (`hasFetched && restaurants.length > 0`), not on the first signed-in page load, and moved inside `RestaurantProvider` to read that. Also portalled to `document.body`: it was the only modal in the codebase that was not, which is the likely cause of its buttons not responding. /demo verified in-browser this time — locked rows carry `videoUrl: ""` over the wire, the card modal opens portalled with PiP, and closing restores scroll.)
