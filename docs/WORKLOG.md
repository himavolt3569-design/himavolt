...
- **Last updated**: 2026-08-17 (Master admin is no longer rate-limited out of its own login — credentials are compared before the limiter and a correct master pair skips it. Platform staff and wrong credentials still get 5/15min, verified live (401s then 429 with Retry-After). **This removes the lockout as a brute-force control on the master password**; a 300ms constant delay on every failure stands in for it, and `MASTER_ADMIN_PASSWORD` is now the whole boundary on the root account — see docs/03.)
