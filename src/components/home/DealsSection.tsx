...
export default function DealsSection() {
  ...
  /* GSAP: deal cards entrance + banner text stagger */
  useGSAP(
    () => {
      if (!sectionRef.current) return;

      /* Banner text elements */
      const bannerEls = gsap.utils.toArray<HTMLElement>(".deals-banner-el");
      ...
      <div className="mx-auto max-w-[1440px] px-4 md:px-8 lg:px-12 py-16 md:py-24">
        ...
        >
          ...
          <div className="relative z-10 max-w-lg">
            <div className="deals-banner-el inline-flex items-center gap-2 rounded-full bg-white/15 backdrop-blur-sm px-4 py-2 text-[11px] font-bold text-white/90 uppercase tracking-wider border border-white/10 mb-5">
              <Sparkles className="h-3.5 w-3.5" />
              Limited time offers
            </div>
            <h3 className="deals-banner-el text-2xl sm:text-3xl md:text-4xl font-extrabold text-white tracking-tight leading-[1.1] mb-3">
              Deals that make
              <br />
              <span className="text-white/70">your wallet smile.</span>
            </h3>
            <p className="deals-banner-el text-sm text-white/50 font-medium mb-6 max-w-md">
              Save big on your favourite meals. New deals every week across 100+
              restaurants in Kathmandu.
            </p>
            <Link
              href="/offers"
              className="deals-banner-el group inline-flex items-center gap-2.5 rounded-full bg-white px-6 py-3 text-sm font-bold text-[#3e1e0c] shadow-lg shadow-black/20 transition-all hover:shadow-xl hover:-translate-y-0.5 active:translate-y-0"
            >
              View All Offers
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {deals.map((deal) => (
            <motion.div
              ...
              className={`deal-card group relative rounded-2xl border ${deal.border} ${deal.bg} p-6 transition-all duration-300 hover:shadow-lg hover:shadow-[#eaa94d]/8 cursor-pointer`}
            >
              <motion.div
