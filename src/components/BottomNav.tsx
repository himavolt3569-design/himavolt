...
const NAV_ITEMS = [
  ...
  { name: "Profile", href: "/#", icon: User }, // Mock profile link
];

export default function BottomNav() {
  const pathname = usePathname();

  // Don't show on scan page or larger screens
  if (pathname === "/scan" || pathname === "/dashboard") {
    // We might want to show a specialized nav on dashboard, but for now we hide it
    // Wait, the prompt says "Bottom nav bar on mobile (Home, Menu, Orders, Profile)"
    // Let's show it on dashboard too if they are viewing as user (but /dashboard is owner view)
    // Actually, let's just render it always on mobile unless it's /scan
