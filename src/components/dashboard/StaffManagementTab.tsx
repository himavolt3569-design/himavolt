...
const ROLE_COLORS: Record<StaffRole, { bg: string; text: string; border: string }> = {
  ...
  CHEF: { bg: "bg-orange-50", text: "text-orange-700", border: "border-orange-200" },
  WAITER: { bg: "bg-emerald-50", text: "text-emerald-700", border: "border-emerald-200" },
  CASHIER: { bg: "bg-amber-50", text: "text-amber-700", border: "border-amber-200" },
};
          ...
          <div className="flex items-center gap-3">
            ...
            {restaurant.restaurantCode && (
              <div className="flex items-center gap-1.5 rounded-lg bg-emerald-50 border border-emerald-100 px-3 py-1">
                ...
                <span className="text-xs font-bold text-emerald-800 tracking-wider">CODE: <span className="font-mono">{restaurant.restaurantCode}</span></span>
              </div>
            )}
          </div>
          <p className="mt-1 text-sm text-gray-500">
            Manage team members for <strong className="text-[#1F2A2A]">{restaurant.name}</strong>
          </p>
        </div>
        <button
...
function AttendanceLogsView({ restaurantId }: { restaurantId: string }) {
  ...
  if (loading) return <div className="py-10 text-center text-sm font-bold text-gray-400">Loading attendance...</div>;
    ...
    <div className="rounded-2xl bg-white border border-gray-100 p-1 shadow-sm overflow-hidden">
      ...
      {logs.length === 0 ? (
        <div className="flex flex-col items-center py-16 text-center">
          ...
          <p className="text-xs text-gray-400 mt-1">Staff punch records will appear here</p>
        </div>
      ) : (
        <div className="divide-y divide-gray-50">
          {logs.map((log) => (
            <div key={log.id} className="grid grid-cols-[minmax(140px,2fr)_1fr_1fr_1fr] items-center gap-4 px-5 py-3.5 hover:bg-gray-50/50 transition-colors">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-bold text-[#1F2A2A]">{log.staff.user.name}</p>
                  <p className="text-[10px] font-semibold text-gray-500">{log.staff.role}</p>
                </div>
              </div>
          ...
          {staff.length === 0 ? (
            ...
            staff.map((member: any, i: number) => {
              ...
              return (
                ...
                >
                  <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl ${colors.bg}`}>
                    <RoleIcon className={`h-5 w-5 ${colors.text}`} />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <h4 className="font-bold text-[#1F2A2A] truncate">{member.user.name}</h4>
                      <span className={`shrink-0 rounded-md border px-2 py-0.5 text-[10px] font-bold ${colors.bg} ${colors.text} ${colors.border}`}>
                ...
                <div className="rounded-2xl bg-emerald-50 p-6 text-center border border-emerald-100">
                  ...
                  <div className="bg-white rounded-xl p-4 shadow-sm space-y-3">
                    ...
                    <div className="flex justify-between items-center pt-1">
                      <span className="text-xs font-bold tracking-wider text-gray-500 uppercase">Login PIN</span>
