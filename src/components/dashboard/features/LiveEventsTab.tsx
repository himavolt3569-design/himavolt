...
const EVENT_TYPE_CONFIG: Record<EventType, { label: string; icon: typeof Music; color: string; bg: string }> = {
  ...
  "comedy": { label: "Comedy Night", icon: Laugh, color: "text-[var(--accent)]", bg: "bg-[var(--accent-muted)]" },
  "open-mic": { label: "Open Mic", icon: Radio, color: "text-blue-500", bg: "bg-blue-50" },
  "sports": { label: "Sports Screening", icon: Tv, color: "text-[var(--accent-hover)]", bg: "bg-[var(--accent-muted)]" },
};
...
const STATUS_COLORS: Record<EventStatus, { label: string; color: string; bg: string }> = {
  ...
  ongoing: { label: "Ongoing", color: "text-[var(--accent-text)]", bg: "bg-[var(--accent-muted)]" },
    ...
    <div className="max-w-5xl mx-auto space-y-6 pb-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[var(--text-2)] mt-1">Manage music nights, shows, and event listings</p>
        </div>
        <button
        ...
        {(["all", "upcoming", "ongoing", "completed", "cancelled"] as const).map((f) => (
          <button
            ...
            className={`rounded-lg px-3 py-1.5 text-xs font-semibold capitalize transition-all ${
              filter === f ? "bg-rose-100 text-rose-700" : "bg-[var(--canvas-sub)] text-[var(--text-2)] hover:bg-[var(--surface)]"
            }`}
          >
            {f}
          </button>
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
            <div className="rounded-xl bg-[var(--canvas)] ring-1 ring-[var(--border)] p-5 shadow-md space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-bold text-[var(--text-1)]">Create New Event</h3>
                <button onClick={() => setShowCreate(false)} className="text-[var(--text-3)] hover:text-[var(--text-2)]"><X className="h-4 w-4" /></button>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-[var(--text-2)] mb-1">Event Name</label>
                  <input type="text" value={newEvent.name} onChange={(e) => setNewEvent({ ...newEvent, name: e.target.value })} placeholder="e.g., Friday Jazz Night" className="w-full rounded-lg bg-[var(--canvas-sub)] px-3 py-2.5 text-sm ring-1 ring-[var(--border)] outline-none focus:ring-rose-400" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-[var(--text-2)] mb-1">Type</label>
                  <select value={newEvent.type} onChange={(e) => setNewEvent({ ...newEvent, type: e.target.value as EventType })} className="w-full rounded-lg bg-[var(--canvas-sub)] px-3 py-2.5 text-sm ring-1 ring-[var(--border)] outline-none focus:ring-rose-400">
                ...
                <div>
                  ...
                  <input type="date" value={newEvent.date} onChange={(e) => setNewEvent({ ...newEvent, date: e.target.value })} className="w-full rounded-lg bg-[var(--canvas-sub)] px-3 py-2.5 text-sm ring-1 ring-[var(--border)] outline-none focus:ring-rose-400" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-[var(--text-2)] mb-1">Performer / Act</label>
                  <input type="text" value={newEvent.performer} onChange={(e) => setNewEvent({ ...newEvent, performer: e.target.value })} placeholder="e.g., DJ Prashant" className="w-full rounded-lg bg-[var(--canvas-sub)] px-3 py-2.5 text-sm ring-1 ring-[var(--border)] outline-none focus:ring-rose-400" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-[var(--text-2)] mb-1">Start Time</label>
                  <input type="time" value={newEvent.startTime} onChange={(e) => setNewEvent({ ...newEvent, startTime: e.target.value })} className="w-full rounded-lg bg-[var(--canvas-sub)] px-3 py-2.5 text-sm ring-1 ring-[var(--border)] outline-none focus:ring-rose-400" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-[var(--text-2)] mb-1">End Time</label>
                  <input type="time" value={newEvent.endTime} onChange={(e) => setNewEvent({ ...newEvent, endTime: e.target.value })} className="w-full rounded-lg bg-[var(--canvas-sub)] px-3 py-2.5 text-sm ring-1 ring-[var(--border)] outline-none focus:ring-rose-400" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-[var(--text-2)] mb-1">Cover Charge ({getCurrencySymbol(cur)})</label>
                  <input type="number" value={newEvent.coverCharge} onChange={(e) => setNewEvent({ ...newEvent, coverCharge: e.target.value })} placeholder="0 for free" className="w-full rounded-lg bg-[var(--canvas-sub)] px-3 py-2.5 text-sm ring-1 ring-[var(--border)] outline-none focus:ring-rose-400" />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-[var(--text-2)] mb-1">Capacity</label>
                  <input type="number" value={newEvent.capacity} onChange={(e) => setNewEvent({ ...newEvent, capacity: e.target.value })} placeholder="50" className="w-full rounded-lg bg-[var(--canvas-sub)] px-3 py-2.5 text-sm ring-1 ring-[var(--border)] outline-none focus:ring-rose-400" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-semibold text-[var(--text-2)] mb-1">Description</label>
                <textarea value={newEvent.description} onChange={(e) => setNewEvent({ ...newEvent, description: e.target.value })} rows={2} placeholder="Event details..." className="w-full rounded-lg bg-[var(--canvas-sub)] px-3 py-2.5 text-sm ring-1 ring-[var(--border)] outline-none focus:ring-rose-400 resize-none" />
              </div>
              <div className="flex items-center justify-between">
                <label className="flex items-center gap-2 text-xs font-semibold text-[var(--text-2)] cursor-pointer">
                ...
                <button onClick={handleCreate} disabled={!newEvent.name || !newEvent.date} className="flex items-center gap-2 rounded-lg bg-rose-500 px-5 py-2 text-sm font-semibold text-white hover:bg-rose-400 disabled:bg-[var(--surface-alt)] disabled:text-[var(--text-3)] transition-all">
      ...
      <div className="space-y-3">
        {filtered.map((event) => {
          ...
          return (
            <motion.div key={event.id} layout className={`rounded-xl bg-[var(--canvas)] ring-1 ring-[var(--border)] p-4 shadow-sm ${event.status === "cancelled" ? "opacity-50" : ""}`}>
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1">
                    <h3 className="text-sm font-bold text-[var(--text-1)]">{event.name}</h3>
                  ...
                  {event.performer && <p className="text-xs text-[var(--text-2)] mb-1.5">Featuring: {event.performer}</p>}
                ...
                <div className="text-right shrink-0">
                  <div className="flex items-center gap-1.5 mb-1">
                    <div className="h-2 w-16 rounded-full bg-[var(--surface)] overflow-hidden">
                      <div className={`h-full rounded-full ${occupancy > 80 ? "bg-[var(--accent)]" : occupancy > 50 ? "bg-[var(--accent)]" : "bg-[var(--border)]"}`} style={{ width: `${occupancy}%` }} />
                    </div>
                    <span className="text-[10px] text-[var(--text-2)]">{occupancy}%</span>
                  </div>
                  <p className="text-xs text-[var(--text-2)]">{event.ticketsSold}/{event.capacity} sold</p>
                  {event.revenue > 0 && <p className="text-xs font-bold text-[var(--accent-hover)] mt-0.5">{formatPrice(event.revenue, cur)}</p>}
                </div>
              </div>
              {event.status === "upcoming" && (
                <div className="flex gap-2 mt-3 pt-3 border-t border-[var(--border-soft)]">
                  <button onClick={() => handleStatusChange(event.id, "ongoing")} className="text-xs font-semibold text-[var(--accent-text)] bg-[var(--accent-muted)] px-3 py-1.5 rounded-lg hover:bg-[var(--accent-muted)] transition-all">Start Event</button>
      ...
      {filtered.length === 0 && (
        <div className="text-center py-16">
          <Music className="h-10 w-10 text-[var(--text-3)] mx-auto mb-3" />
          <p className="text-sm text-[var(--text-3)]">No events found</p>
        </div>
      )}
    </div>
  );
