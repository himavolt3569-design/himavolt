                ...
                <div className="flex items-center gap-2 mb-0.5">
                  <span
                    className={`flex h-2.5 w-2.5 rounded-full ${
                      order.status === "new" ? "bg-[#FF9933] animate-pulse" : "bg-[#0A4D3C]"
                    }`}
                  />
                  <span className="text-[11px] font-bold uppercase tracking-wider text-gray-400">
                    {order.status === "new" ? "New Dine-In Request" : `Order ${order.status}`}
                  </span>
                </div>
                <h2 className="text-lg font-bold text-[#1F2A2A]">{order.id}</h2>
              </div>
              <button
                onClick={onClose}
