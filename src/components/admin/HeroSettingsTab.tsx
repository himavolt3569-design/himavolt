    ...
    <div className="max-w-5xl">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="rounded-3xl border border-blue-100 bg-[var(--canvas)] p-5 shadow-sm">
            ...
            {settings.images.length > 0 ? (
              ...
              >
                {settings.images.map((image) => (
                  <Reorder.Item
                    key={image.id}
                    value={image}
                    className="group relative flex items-center gap-3 rounded-xl border border-white/40 shadow-sm bg-[var(--surface)]/60 backdrop-blur-xl p-2 shadow-sm"
                  >
                    <div className="cursor-grab p-1 text-[var(--text-3)] hover:text-[var(--text-3)] active:cursor-grabbing">
                    ...
                    <button
                      ...
                      className="rounded-lg p-2 text-[var(--text-3)] hover:bg-red-50 hover:text-red-500 transition-colors"
                    >
              ...
              <div className="rounded-xl border-2 border-dashed border-white/40 shadow-sm p-8 text-center">
                ...
                <p className="mt-2 text-sm text-[var(--text-3)]">No images yet</p>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  {settings.autoplay ? (
                    ...
                    <Pause className="h-4 w-4 text-[var(--text-3)]" />
                  )}
                  <span className="text-sm text-[var(--text-2)]">Autoplay</span>
                </div>
                <button
                  onClick={() =>
                  ...
                  className={`relative h-6 w-11 rounded-full transition-colors ${
                    settings.autoplay ? "bg-blue-500" : "bg-[var(--border-soft)]"
                  }`}
                >
                  <span
                    className={`absolute top-1 h-4 w-4 rounded-full bg-[var(--surface)]/60 backdrop-blur-xl transition-transform ${
              ...
              <div>
                <div className="mb-2 flex items-center justify-between">
                  <span className="text-sm text-[var(--text-2)]">Slide Interval</span>
                  <span className="text-xs text-[var(--text-3)]">
                ...
                <input
                  ...
                  className="h-2 w-full cursor-pointer appearance-none rounded-lg bg-[var(--border-soft)] accent-blue-500"
                />
              ...
              <div>
                <div className="mb-2 flex items-center justify-between">
                  <span className="text-sm text-[var(--text-2)]">Overlay Opacity</span>
                  <span className="text-xs text-[var(--text-3)]">
        ...
        <div className="space-y-4">
          <div className="rounded-3xl border border-blue-100 bg-[var(--canvas)] p-5 shadow-sm">
            ...
            {/* Preview Container */}
            <div className="relative aspect-[16/10] overflow-hidden rounded-xl bg-[var(--surface-alt)]">
              {settings.images.length > 0 ? (
                  ...
                  {settings.images.map((_, i) => (
                    <button
                      ...
                      className={`h-1.5 rounded-full transition-all ${
                        i === previewIndex
                          ? "w-6 bg-[var(--surface)]/60 backdrop-blur-xl"
                          : "w-1.5 bg-[var(--surface)]/60 backdrop-blur-xl/50 hover:bg-[var(--surface)]/60 backdrop-blur-xl/75"
                      }`}
                    />
                  ))}
                </div>
            ...
            {settings.images.length > 1 && (
              <div className="mt-3 flex items-center justify-center gap-2">
                <button
                  ...
                  className="rounded-lg bg-[var(--surface-alt)] px-3 py-1.5 text-xs font-medium text-[var(--text-2)] hover:bg-[var(--border-soft)]"
                >
                  Previous
                </button>
                <span className="text-xs text-[var(--text-3)]">
          ...
          {/* Tips */}
          <div className="rounded-xl bg-blue-50/50 p-4 text-xs text-[var(--text-3)]">
      ...
      <div className="mt-6 flex items-center gap-3">
        ...
        <button
          onClick={handleReset}
          disabled={saving}
          className="flex items-center gap-2 rounded-xl border border-white/40 shadow-sm bg-[var(--surface-alt)]/50 px-4 py-2.5 text-sm font-medium text-[var(--text-3)] transition-all hover:bg-[var(--surface-alt)] disabled:opacity-50"
        >
