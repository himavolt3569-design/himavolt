...
export default function AllRestaurantsTab() {
  ...
  useEffect(() => {
    if (!loading) fetchRestaurants(page);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        {pagination && (
          <span className="ml-auto text-xs text-gray-400">{pagination.total} restaurants</span>
        )}
      </div>
        ...
        {loading && restaurants.length === 0 ? (
          ...
          <div className="divide-y divide-gray-50">
            {restaurants.map((r) => {
              ...
              return (
                <div key={r.id} className="transition-all hover:bg-brand-50/40">
                  <button
                    type="button"
                    onClick={() => setExpandedId(isExpanded ? null : r.id)}
                    className="flex w-full items-center gap-3 px-4 py-3 text-left"
                  >
                    <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-brand-50 overflow-hidden">
                      {r.imageUrl ? (
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-3 text-xs text-gray-500">
                        ...
                        <span className="flex items-center gap-1"><ShoppingBag className="h-3 w-3" />{r._count.orders} orders</span>
                        <span className="flex items-center gap-1"><Users className="h-3 w-3" />{r._count.staff} staff</span>
                        {r.rating > 0 && (
                          <span className="flex items-center gap-0.5"><Star className="h-3 w-3 fill-amber-400 text-amber-400" />{r.rating.toFixed(1)}</span>
                        )}
                      </div>
                    </div>
                        ...
                        <div className="border-t border-brand-100 bg-brand-50/30 px-4 py-3 space-y-3">
                          <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-xs sm:grid-cols-4">
                            <div>
                              ...
                              <p className="font-mono text-gompa-slate">{r.slug}</p>
                            </div>
                            <div>
                              <span className="text-gray-400">Code</span>
                              <p className="font-mono text-gompa-slate">{r.restaurantCode}</p>
                            </div>
                            <div>
                              <span className="text-gray-400">Phone</span>
                              <p className="font-medium text-gompa-slate">{r.phone}</p>
                            </div>
                            <div>
                              <span className="text-gray-400">Currency</span>
                              <p className="font-medium text-gompa-slate">{r.currency}</p>
                            </div>
                            <div>
                              <span className="text-gray-400">Menu Items</span>
                              <p className="font-medium text-gompa-slate">{r._count.menuItems}</p>
                            </div>
                            <div>
                              <span className="text-gray-400">Reviews</span>
                              <p className="font-medium text-gompa-slate">{r._count.reviews}</p>
                            </div>
                            <div>
                              <span className="text-gray-400">Total Orders</span>
                              <p className="font-bold text-gompa-slate">{r.totalOrders}</p>
                            </div>
                            <div>
                              <span className="text-gray-400">Created</span>
                              <p className="font-medium text-gompa-slate">{new Date(r.createdAt).toLocaleDateString()}</p>
                            </div>
                            <div className="col-span-2 sm:col-span-4">
                              <span className="text-gray-400">Address</span>
                              <p className="font-medium text-gompa-slate">{r.address}</p>
                            </div>
                            <div className="col-span-2 sm:col-span-4">
                              <span className="text-gray-400">Owner</span>
                              <p className="font-medium text-gompa-slate">{r.owner.name} ({r.owner.email})</p>
                            </div>
                          </div>
