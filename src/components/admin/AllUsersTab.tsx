...
export default function AllUsersTab({
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        queryClient.setQueryData<typeof usersQuery.data>(usersQueryKey, (prev: { users: UserRecord[]; pagination: Pagination } | undefined) =>
          prev
            ? {
                users: prev.users.filter((u: UserRecord) => u.id !== deleteTarget.id),
    ...
    try {
      ...
      if (res.ok) {
        queryClient.setQueryData<typeof usersQuery.data>(usersQueryKey, (prev: { users: UserRecord[]; pagination: Pagination } | undefined) =>
          prev
            ? {
                users: prev.users.filter((u: UserRecord) => !selectedIds.has(u.id)),
  ...
  const changeRole = async (userId: string, role: string) => {
    ...
    queryClient.setQueryData<typeof usersQuery.data>(usersQueryKey, (prev: { users: UserRecord[]; pagination: Pagination } | undefined) =>
      prev ? { ...prev, users: prev.users.map((u: UserRecord) => (u.id === userId ? { ...u, role } : u)) } : prev,
    );
    try {
      const res = await fetch("/api/admin/users", {
        method: "PATCH",
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3 px-2">
        <input
          ...
          checked={allSelected}
          onChange={() =>
            setSelectedIds(allSelected ? new Set() : new Set(users.map((u: UserRecord) => u.id)))
          }
