...
const TABS: { id: AdminTab; label: string; icon: any; category: string }[] = [
  ...
  { id: "gateway-settings", label: "Payment Gateways", icon: CreditCard, category: "System" },
...
function AdminLoginGate({ onSuccess }: { onSuccess: () => void }) {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-[#F7F9FC] p-4 relative overflow-hidden font-sans">
              ...
              >
                <div className="relative mb-6">
                  <motion.div
                    ...
                    className="h-20 w-20 rounded-3xl bg-[var(--accent)] flex items-center justify-center shadow-[0_20px_40px_-10px_var(--accent)]"
                  >
...
/* ═══════════════════════════════════════════════════════════════════
   Beautiful Master Admin Dashboard
   ═══════════════════════════════════════════════════════════════════ */
...
export default function MasterAdminPage() {
  ...
  return (
    <div className="min-h-screen bg-[#F7F9FC] text-gray-900 flex overflow-hidden font-sans">
            ...
            <div key={category}>
              ...
              <div className="space-y-1">
                {TABS.filter(t => t.category === category).map((t) => (
                  ...
                  >
                    <t.icon className={`h-5 w-5 ${tab === t.id ? "text-white" : "text-gray-400"}`} />
                    <span>{t.label}</span>
                  </button>
                ))}
              </div>
            ...
            >
              <div className="mb-8 flex items-end justify-between">
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-500 mt-1">Manage and monitor your operations</p>
                </div>
              </div>
          ...
          <>
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto py-4 px-4 space-y-6">
                {categories.map(category => (
                  <div key={category}>
                    <h3 className="px-3 text-[11px] font-bold uppercase tracking-widest text-gray-400 mb-2">{category}</h3>
                    <div className="space-y-1">
                      {TABS.filter(t => t.category === category).map((t) => (
                        <button
                          key={t.id}
                          onClick={() => handleSetTab(t.id)}
                          className={`w-full flex items-center gap-3 px-3 py-3 rounded-2xl transition-all font-semibold ${
                            tab === t.id 
