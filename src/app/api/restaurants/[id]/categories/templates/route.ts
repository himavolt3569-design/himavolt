...
// GET — list this restaurant's category templates (per its type), flagged
// with whether each one has already been added, so the picker can grey
// out/hide ones already on the menu.
export async function GET(
  ...
  const restaurant = await db.restaurant.findUnique({ where: { id }, select: { type: true } });
  ...
  return NextResponse.json(
    templates.map((t) => ({ ...t, slug: toSlug(t.name), added: existingSlugs.has(toSlug(t.name)) })),
    { headers: { "Cache-Control": "no-store" } },
  );
}

// POST — add a single template category (with its subcategories). Idempotent:
// re-adding an already-added template is a no-op for anything that exists.
export async function POST(
  ...
  const restaurantData = await db.restaurant.findUnique({ where: { id }, select: { type: true } });
  ...
  try {
    ...
    let subSort = 1;
    for (const subName of template.subs) {
      const subSlug = `${parentSlug}--${toSlug(subName)}`;
      const existingSub = await db.menuCategory.findFirst({ where: { restaurantId: id, slug: subSlug } });
      if (!existingSub) {
        await db.menuCategory.create({
          data: { name: subName, slug: subSlug, sortOrder: subSort, restaurantId: id, parentId: parent.id },
        });
      }
      subSort++;
    }
